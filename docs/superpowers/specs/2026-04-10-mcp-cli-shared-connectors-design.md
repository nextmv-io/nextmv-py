...
**CLI-only options** (e.g., `OutputOption` for `--output/-o` save-to-file) omit
the `Field(...)` metadata and are never imported on the MCP side.
...
With options on the action, the connector files become pure wiring:
...
2. Build a new parameter list: all action parameters *except* `client`, with
   ...
   `typer.Option` markers unchanged.
3. Append `profile: ProfileOption = None`. If `output_flag=True`, append
...
4. Define a wrapper function that:
   ...
   - If `output_flag=True` and `output` is truthy, writes JSON to file and
     prints a generic save-success message. Else applies `on_success`
     handling (see Section "Result Handling").
5. Sets `wrapper.__signature__`, `wrapper.__annotations__`,
   ...
   tracebacks), and `wrapper.__doc__ = action.__doc__ + "\n\n" + help_extra`
   (if `help_extra` is set).
...
When `output_flag=True` and the user passes `--output <path>`, the CLI wrapper
writes JSON to the file and prints a generic save-success message, bypassing
`on_success`.
...
nextmv/cli/framework/
    __init__.py          # re-exports: command, progress, emit,
...
nextmv/cli/mcp/framework/
    __init__.py          # re-exports: tool, client, clean
    tool.py              # mcp.tool() implementation
```
...
- Assert the wrapper has the expected `__signature__` and `__annotations__`.
...
- All existing tests must pass unchanged. They speak to the observable
  contract (flags, output format, tool schemas), which this refactor
  preserves exactly.
...
- `tests/cli/cloud/app/test_help_output.py`:
  ...
  - Assert the help text contains expected flag names, short forms,
    envvar hints, and example blocks.

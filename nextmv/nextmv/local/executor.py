...
# A run_id is generated by `nextmv.safe.safe_id` and is only ever
# `<prefix>-<random>`. It is interpolated into file names (e.g. `<run_id>.json`),
# so it must never contain path separators or traversal sequences.
_SAFE_RUN_ID = re.compile(r"^[A-Za-z0-9._-]+$")


def _validate_execution_input(data: dict[str, Any]) -> None:
    """
    Validate the execution payload received on stdin before any of its values
    are used to build filesystem paths.

    The payload is produced by the trusted local runner
    ...
    for key, expected_type in required.items():
        if key not in data:
            raise ValueError(f"missing required execution input field: {key!r}")
        if not isinstance(data[key], expected_type):
            raise ValueError(f"execution input field {key!r} must be of type {expected_type.__name__}")

    run_id = data["run_id"]
    if ".." in run_id or not _SAFE_RUN_ID.fullmatch(run_id):
        raise ValueError(
...
def main() -> None:
    ...
    input = load()
    _validate_execution_input(input.data)
    execute_run(

...
def register(mcp: MCPServer) -> None:
    ...
    # This tool is async on purpose, unlike its neighbours. The SDK runs sync
    # handlers on a worker thread, which receives a *copy* of the caller's
    # context, so the `ContextVar` write below would be discarded as soon as the
    # handler returned. An async handler stays on the event loop, where the
    # write is visible to the calls that follow. Readers can stay sync: a worker
    # thread inherits the context, so it observes the profile set here.
    @mcp.tool()
    async def cloud_set_profile(profile: str) -> str:

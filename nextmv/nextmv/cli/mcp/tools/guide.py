...
`result_value` (objective value).
  - **ML classifier apps**: use `accuracy`, `precision`, `recall`, \
`f1_score`, `roc_auc`, `log_loss`, and confusion matrix counts. \
Do NOT use `status`/`result_value`.
  - **ML regression / forecasting apps**: use `rmse`, `mae`, `r2`, \
and domain-relevant metrics. Do NOT use `status`/`result_value`.
  - **Simulation / rules-engine apps**: use domain-specific metrics \
(throughput, utilization, queue length, etc.).
- **Visualization**: Every app must include at least one Plotly \
visualization rendered as a `nextmv.Asset`.
- **Configuration**: Every app must expose user-facing options via \
`app.yaml`. Apps that use a random seed must expose a `random_seed` \
option (`option_type: int`, not required, no default).
...
Input is a single JSON file via stdin; output is a single JSON object \
via stdout.
...
The `input` and `output` options are required so that \
`nextmv.load(options=options, path=options.input)` works correctly.

### Multi-file format

Input and output are directories of files. Use when the solver writes \
its own output files or when you need CSV/Excel/text output.
...
Read input files directly; write output files to disk. Do NOT call \
`nextmv.load`.
...
Include `input.json` (default) plus an `inputs/` directory with 3-5 \
files varying problem size, constraint tightness, or scenario type.
...
local_run(app_dir=".", input=<parsed inputs/large.json>, \
...
Include these sections: App Name (one-sentence description), Approach, \
Configuration Options (table), Input Format, Output (solution, metrics, \
visualizations), Running Locally (tool calls), Syncing to Cloud.
...
```
cloud_create_instance(app_id="<app-id>", version_id="<ver>", \
...
```
cloud_create_input_set(app_id="<app-id>", name="Test inputs", \
instance_id="latest", maximum_runs=10)
```

### 6. Run a scenario test

```
cloud_create_scenario_test(app_id="<app-id>", name="Smoke test", \
scenarios=[{"instance_id": "staging", "scenario_input": \
...
```
cloud_create_acceptance_test(app_id="<app-id>", \
candidate_instance_id="staging", baseline_instance_id="production", \
input_set_id="<input-set-id>", metrics=[{"field": "result_value", \
"metric_type": "direct-comparison", "statistic": "mean", \
"params": {"operator": "le"}}])
```
...
```
cloud_update_instance(app_id="<app-id>", instance_id="production", \
version_id="<new-ver>")
```

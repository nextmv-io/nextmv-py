"""CLI-only workflows for the cloud app domain.

These functions implement interactive workflows that go beyond the thin
action layer in ``nextmv.cli.actions.app``. They are consumed by the
connector table in ``nextmv/cli/cloud/app/__init__.py`` via
``cli.command(..., handles_own_output=True)``.

The MCP side does NOT use this module -- MCP wraps the thin actions in
``cli.actions.app`` directly.
"""
...
from nextmv.cli.message import confirmation, error, in_progress, info, success, warning
...
def run_push_workflow(
    ...
    cloud_app = _build_cloud_app_with_client(client=client, app_id=app_id)
...
def _build_cloud_app_with_client(client: Client, app_id: str) -> Application:
    """
    Build a ``cloud.Application`` using an injected client, preserving the
    interactive "create if missing" behavior of
    ``nextmv.cli.configuration.config.build_cloud_app``.

    If the application does not exist, the user is prompted to create it. If
    they decline, the command exits with an error.

    Parameters
    ----------
    client : Client
        The injected cloud client.
    app_id : str
        The application ID.
